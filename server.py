...
class PromptServer():
    def __init__(self, loop):
        ...
        @routes.get('/ws_prompt')
        async def websocket_prompt_handler(request):
            logging.info("websocket prompt connection")
            ...
            if json_str == '':
                await self.send("prompt_error", { "status": "no prompt", 'sid': sid }, sid)
                return ws
        ...
        async def get_history(request):
            logging.info("get history by id: {}".format(request.match_info.get("prompt_id", None)))
        ...
        async def post_interrupt(request):
            logging.info("post interrupt")
            nodes.interrupt_processing()
            return web.Response(status=200)

        @routes.post("/free")
        async def post_free(request):
            logging.info("post free")
            json_data = await request.json()
    ...
    def _post_prompt(self, json_data):
        ...
        if "prompt" in json_data:
            ...
            if valid[0]:
                ...
                logging.warning("invalid prompt: {}".format(valid[1]))
                return {"error": valid[1], "node_errors": valid[3]}, 400
        else:
            return {"error": "no prompt", "node_errors": []}, 400   
